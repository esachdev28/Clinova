...
Advanced Decision Intelligence Terminal with Strategy Sandbox.
"""
...
def get_intelligence_data(summary_type="Rule-based"):
    ...
    # 1. Advanced Charts
    asset = list(obs["asset_prices"].keys())[0]
    fig_candle = build_candlestick_chart(asset, history)
    fig_pnl = build_pnl_chart(history)
    fig_pie = build_allocation_pie(obs["current_positions"], obs["asset_prices"])
    fig_risk = build_risk_gauge(obs["risk_exposure_score"])
    ...
    return (
        obs["portfolio_value"], obs["unrealized_pnl"], obs["volatility_index"],
...
with gr.Blocks(title="FinomIQ Terminal") as demo:
    ...
    def update_terminal(m, a, r, h, s_mode):
        ...
        # results contains 18 values, but run_btn.click outputs expects 19 (including log_viewer)
        # We need to add log_viewer output value
        logs = get_logs()
        return (*results, logs)
...
if __name__ == "__main__":
    ...
    demo.launch(server_port=args.server_port, theme=FinomIQTheme())
